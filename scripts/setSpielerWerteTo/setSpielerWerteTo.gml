//neutrale Werte
geschwindigkeit = gaussBetween(argument0,argument1, 0, 100);
zweikampf = gaussBetween(argument0,argument1, 0, 100);
stellungsspiel = gaussBetween(argument0,argument1, 0, 100);
ausdauer = gaussBetween(argument0,argument1, 0, 100);
kopfball = gaussBetween(argument0,argument1, 0, 100);
//mentale Werte
selbstbewusstsein = gaussBetween(argument0,argument1, 0, 100);
disziplin = gaussBetween(argument0,argument1, 0, 100);
aggresivitat = gaussBetween(argument0,argument1, 0, 100);
//defensive Werte
antizipation = gaussBetween(argument0,argument1, 0, 100);
torwart = gaussBetween(argument0 - 15,argument1, 0, 100);
//offensive Werte
schuss = gaussBetween(argument0,argument1, 0, 100);
freistoss = gaussBetween(argument0,argument1, 0, 100);
pass = gaussBetween(argument0,argument1, 0, 100);
dribbling = gaussBetween(argument0,argument1, 0, 100);
flanken = gaussBetween(argument0,argument1, 0, 100);
//aktive Werte (default 100)
moral = 100;
puste = 100;